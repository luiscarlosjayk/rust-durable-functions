...
async fn function_handler(
    ...
    info!(
        ...
        "Starting Claude Code CLI execution"
    );
            ...
            move |_| {
                tokio::task::block_in_place(|| {
                    tokio::runtime::Handle::current().block_on(async {
                        ...
                        info!("Spawning claude CLI subprocess");

                        // Run through shell to capture all output
                        let shell_cmd = format!(
                            "{} -p '{}' --output-format json --model claude-haiku-4-5-20251001 --max-turns {} --max-budget-usd {} 2>&1",
                            claude_bin,
                            prompt.replace('\'', "'\\''"),
                            max_turns,
                            max_budget,
